...
function isTitle(line) {
  var titleLine = /^=+\s+([^\s].*)/;
  return getMatches(titleLine, line);
}
...
function isAttribute(line) {
  var attrLine = /^\[\s*([^\[\]]*)\]\s*$/;
  return getMatches(attrLine, line);
}

function isExampleBlock(line) {
  var blockLine = /^====/;
  return getMatches(blockLine, line);
}
  ...
  if (len !== 1) {
    ...
    switch (chara) {
      case ":" :
          ...
          var result = isSubstitution(line);
          if (result.length === 5) {
            if (result.length !== 5) {
              throw {
      ...
      case "=" :
          ...
          if (result$1.length === 2) {
            ...
            var title = result$1[1];
            console.log("TITLE: " + title);
          } else {
            var result$2 = isExampleBlock(line);
            if (result$2.length === 1) {
              console.log("Start of Example block");
            }
...
exports.isExampleBlock = isExampleBlock;
