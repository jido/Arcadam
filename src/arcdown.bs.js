...
function consumeExampleBlock(lnum, subs, attrs) {
  ...
  var l = lnum + 1 | 0;
  while(getMatches(blockLine, Belt_Array.get(lines, l - 1 | 0)).length === 0 && l < lines.length) {
    console.log("|| " + Belt_Option.getWithDefault(Belt_Array.get(lines, l - 1 | 0), "."));
    l = l + 1 | 0;
  };
  return [
          l + 1 | 0,
...
while(lnum.contents <= lines.length) {
  var line = Belt_Array.get(lines, lnum.contents - 1 | 0);
  var firstChar = /^./;
  ...
  if (len !== 1) {
    if (len !== 0) {
      ...
      lnum.contents = lnum.contents + 1 | 0;
    } else {
      console.log("<empty>");
      lnum.contents = lnum.contents + 1 | 0;
    }
    ...
    switch (chara) {
      case ":" :
          ...
          var match = consumeSubstitution(lnum.contents, subs.contents);
          var next = match[0];
      ...
      case "=" :
          ...
          var match$1 = consumeTitle(lnum.contents, subs.contents);
          var next$1 = match$1[0];
          if (next$1 > lnum.contents) {
            lnum.contents = next$1;
            subs.contents = match$1[1];
          } else {
            var match$2 = consumeExampleBlock(lnum.contents, subs.contents, attrs.contents);
            var next$2 = match$2[0];
            if (next$2 > lnum.contents) {
              console.log("End of Example block");
              lnum.contents = next$2;
              subs.contents = match$2[1];
              attrs.contents = "";
            } else {
              lnum.contents = lnum.contents + 1 | 0;
            }
      ...
      case "[" :
          ...
          var match$3 = consumeAttribute(lnum.contents, subs.contents);
          var next$3 = match$3[0];
          if (next$3 > lnum.contents) {
            var attributes = match$3[2];
            console.log("ATTR: " + attributes);
            lnum.contents = next$3;
            subs.contents = match$3[1];
